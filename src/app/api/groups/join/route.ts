...
export async function POST(request: Request) {
  ...
  const invite = await queryOne<{ id: number; group_id: number; max_uses: number | null; used_count: number; expires_at: string | null }>(
    "SELECT id, group_id, max_uses, used_count, expires_at FROM group_invites WHERE token_hash = ?",
    [tokenHash(token)],
  );
