...
export default async function GroupPage({ params, searchParams }: GroupPageProps) {
  ...
  const canModerate = group.viewer_role === "owner" || group.viewer_role === "moderator";
  const locked = group.privacy === "private" && !group.is_member;
                  ...
                  {requests.map((request) => (
                    <article key={request.user_id} className="grid gap-3 border-t border-charcoal/10 pt-3 first:border-t-0 first:pt-0">
                      <div className="flex items-center gap-2">
                        <UserAvatar
                          user={{ name: request.user_name, avatar_url: request.user_avatar_url }}
                          size="sm"
                        />
