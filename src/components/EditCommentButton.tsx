...
export function EditCommentButton({
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (!response.ok) {
        ...
        showToast(payload.error ?? "Commento non modificato.", "error");
        return;
      }
  ...
  return (
    <form onSubmit={submit} className="mt-3 rounded-lg border border-charcoal/10 bg-surface p-3">
      <textarea
        value={content}
        onChange={(event) => setContent(event.target.value)}
        maxLength={500}
        rows={3}
        className="min-h-24 w-full resize-none rounded-lg border border-charcoal/10 bg-paper px-3 py-2 text-sm leading-6 text-charcoal outline-none transition focus:border-fern-700 focus:ring-4 focus:ring-fern-700/10"
      />
